...
class Counter extends Component {
  render() {
    ...
    return (
      <div ref={useRef('ref1')}>
        {this.state.count}
        <button ref={useRef('ref2')} onClick={add}>add</button>
      </div>
    );
  }
}
...
function FnCounter() {
  ...
  return (
    <div ref={ctx.useRef('ref1')}>
      {ctx.state.count}
      <button ref={ctx.useRef('ref2')} onClick={add}>add</button>
    </div>
  );
}
...
export default function App() {
  return (
    <div className="App">
      <h3> capture ref with ctx.useRef in fn component and class component both</h3>
      <Counter />
      <FnCounter />
    </div>
  );
