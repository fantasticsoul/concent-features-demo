...
const setup = ctx => {
  ...
  ctx.computed('count', (n, o, f)=>{
    ...
    return ctx.type === cst.CC_CLASS_PREFIX ? n.count *10 : n.count * 100;
  });

  ctx.watch('countChange', (n, o, f)=>{
    // commit only change private state when called in ref watch
    if(ctx.type === cst.CC_HOOK_PREFIX)f.commit({msg:'count changed '+Date.now()})
  }, ['count']);
...
function FnCounter() {
  ...
  const {
    settings: { add }, refComputed, state
  } = ctx;
